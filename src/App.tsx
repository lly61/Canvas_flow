...
export default function App() {
  ...
  useEffect(() => {
    ...
    const drawStars = () => {
      ...
      for (let i = 0; i < 150; i++) {
        sCtx.beginPath();
        sCtx.arc(Math.random() * cw, Math.random() * ch, Math.random() * 1.5, 0, Math.PI * 2);
    ...
    class Particle {
      ...
      constructor(x: number, y: number, baseHue: number, maxSpeed: number, angle?: number, speed?: number, gravity: number = 0.8) {
        this.x = x;
        this.y = y;
        this.coordinates = [];
    ...
    const createParticles = (x: number, y: number, count: number, baseHue: number, maxSpeed: number) => {
      while (count--) {
    ...
    const createRoseParticles = (x: number, y: number) => {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        particles.push(new Particle(x, y, baseHue, 0, theta, speed, roseGravity));
      }
      ...
      for (let i = 0; i < 150; i++) {
        ...
        particles.push(new Particle(x, y, baseHue - 10, 0, theta, speed, roseGravity));
      }
    ...
    const loop = () => {
      ...
      if (flashDiv) {
        flashDiv.style.backgroundColor = `hsla(${flashHue}, 100%, 70%, ${flashAlpha * 0.3})`;
      }
  ...
  return (
    <div ref={skyRef} className="fixed inset-0 bg-[#050510] overflow-hidden touch-none">
      {/* Stars Background */}
      <canvas ref={starsCanvasRef} className="absolute inset-0 block w-full h-full pointer-events-none" />
      ...
      <div className="absolute top-12 right-12 md:top-20 md:right-24 w-28 h-28 md:w-40 md:h-40 pointer-events-none">
        ...
        <div className="absolute inset-0 rounded-full"
             style={{
               boxShadow: '0 0 60px 20px rgba(255, 255, 220, 0.3), 0 0 100px 40px rgba(255, 255, 255, 0.1)'
             }}>
        </div>
        {/* Moon Image */}
        <div className="absolute inset-0 rounded-full"
             style={{
               backgroundImage: 'url("https://images.unsplash.com/photo-1522030299830-16b8d3d049fe?q=80&w=400&auto=format&fit=crop")',
               backgroundSize: '140%',
      ...
      {/* Fireworks Canvas */}
      <canvas ref={canvasRef} className="absolute inset-0 block w-full h-full pointer-events-auto touch-none" />
      ...
      <div className="absolute top-12 left-0 right-0 text-center pointer-events-none select-none z-10">
        ...
        <p className="text-white/20 text-sm mt-2 tracking-widest">
          Tap or drag to launch fireworks
        </p>
      </div>
    </div>
  );
